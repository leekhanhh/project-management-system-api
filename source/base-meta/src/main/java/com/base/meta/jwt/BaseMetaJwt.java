...
@Slf4j
@Data
public class BaseMetaJwt implements Serializable {
    ...
    public static BaseMetaJwt decode(String input) {
        BaseMetaJwt result = null;
        try {
            String[] items = ZipUtils.unzipString(input).split(DELIM, 11);
            if (items.length >= 10) {
                result = new BaseMetaJwt();
